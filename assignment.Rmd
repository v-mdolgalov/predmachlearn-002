...
```{r}
set.seed(222)
rfModel<- train(classe~., data=trsm1[,-c(1,2)], method="rf", prox=TRUE, importance=T) ## small subset of rows but with proper bootstrapping of variables
rfModel
rfModel2<- train(classe~., data=trainingsml[,-c(1,2)], method="rf", prox=TRUE, trControl = trainControl(method = "cv", number = 2), importance=T) ## all rows of training set but with simpler train control algorithm
rfModel2
...
impt1<- importance(rfModel);
str(impt1)
impt1fr<- data.frame(impt1)
tail(impt1fr)
impt1fr <- impt1fr[order(impt1fr$MeanDecreaseGini, decreasing = T),]
plot(impt1fr$MeanDecreaseGini, col="magenta", xlab="Variables (Sorted by Importance)", ylab="Importance, Gini coefficient", main="Variable Importance for Random Forest model")
